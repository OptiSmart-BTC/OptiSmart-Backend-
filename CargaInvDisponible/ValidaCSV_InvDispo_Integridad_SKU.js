...
const logFile = `../../${parametroFolder}/log/LogdeCargaInvDispoCSV.log`; // Cambia esta ruta según la ubicación de tu archivo CSV
const csvPath = `../../${parametroFolder}/reportes/InvDisponible_SKUs_Faltantes.csv`;
...
async function main() {
  ...
  try {
    ...
    const ColeccionRespaldo = db.collection('report_sin_sku_invdisp_vs_sku');
    ...
    const skusNoEncontrados = await InvDispoCollection
      .find({ SKU: { $nin: skus } })
      .toArray();
    ...
    await InvDispoCollection.deleteMany({ _id: { $in: skusNoEncontradosIds } });

    writeToLog(`\tSe encontraron ${skusNoEncontradosIds.length} registros en el "Inventario Disponible" con problemas de Integridad de SKUs`);
