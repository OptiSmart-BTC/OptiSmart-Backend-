...
const parametroFolder = process.argv.slice(2)[1]; // p.ej. 'shrt'

const { decryptData } = require('./DeCriptaPassAppDb');
const { host, puerto } = require('../../Configuraciones/ConexionDB');
const { DBUser, DBPassword } = require(`../../../${parametroFolder}/cfg/dbvars`);

// Base: .../OptiSmart/<parametroFolder>/
const baseUserPath = path.resolve(__dirname, '..', '..', '..', parametroFolder);

// Rutas a log y CSV usando __dirname
const logFile = path.join(baseUserPath, 'log', 'Logs_demanda.log');
const csvPath = path.join(baseUserPath, 'reportes', 'historico_demanda_cantidad_0.csv');
...
async function exportarYEliminarRegistros() {
  ...
  try {
    ...
    const reporteDir = path.join(baseUserPath, 'reportes');
    ...
    writeToLog(`\tERROR en LimpiaEspaciosHistorico_v2.js: ${error}`);
...
function writeToLog(message) {
  try {
    ...
    // Último recurso: al menos que se vea en consola
    console.error('Error al escribir en el log:', e);
    console.log(message);
    return;
  }
  console.log(message);
