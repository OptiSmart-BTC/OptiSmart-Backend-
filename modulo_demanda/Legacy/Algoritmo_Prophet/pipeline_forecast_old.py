...
def ejecutar_pipeline(
    ...
    Pipeline universal:
      ...
      - Por DFU: llama al forecast_router (AUTO/MANUAL o single-model) + métricas
      - Devuelve:
    ...
    # 7) Router config
    # ✅ incluye MANUAL como modo router
    is_router = algo_in in ("auto", "router", "frs", "manual")

    dfu_recommendations = model_params.get("dfu_recommendations") if is_router else None
    dfu_selected_models = model_params.get("dfu_selected_models") if is_router else None  # 👈 NUEVO
    fallback_models = model_params.get("router_fallback") if is_router else [algo_in]

    if is_router:
        src = model_params.get("router_source_collection", "")
        if algo_in == "manual":
    ...
    for i, (Producto, Canal, Ubicacion) in enumerate(combinaciones_validas, start=1):
        ...
        try:
            ...
            # ✅ Corre router (AUTO/MANUAL o SINGLE) y deja que el router complete fechas según modelo elegido
            df_prepared, forecast, audit = run_forecast_router(
                ...
                dfu_selected_models=dfu_selected_models,  # 👈 NUEVO
            ...
            try:
                ...
                # no truena el pipeline por métricas
