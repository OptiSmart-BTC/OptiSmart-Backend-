...
def _is_nested_model_params(params: Any) -> bool:
    ...
    if not isinstance(params, dict):
        return False
    # basta con que exista alguna de estas llaves y sea dict
...
def _merge_params(base: Optional[Dict[str, Any]], override: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Merge shallow (override pisa a base). No hace deep-merge.
    """
    out: Dict[str, Any] = {}
...
def _resolve_router_and_model_params(
    params: Optional[Dict[str, Any]],
    model_key: Optional[str] = None,
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """
    Devuelve:
      router_params: params efectivos para decisiones del router
      model_params:  params efectivos para el modelo específico (model_key)

    - Si params NO es anidado: router_params = params, model_params = params.
    - Si params ES anidado: router_params = merge(global, router)
                             model_params  = merge(global, params[model_key])
    """
    params = params or {}
    if not _is_nested_model_params(params):
        return params, params

    global_params = params.get("global", {}) if isinstance(params.get("global"), dict) else {}
    router_block = params.get("router", {}) if isinstance(params.get("router"), dict) else {}

    router_params = _merge_params(global_params, router_block)

    if not model_key:
        return router_params, router_params

    model_block = params.get(model_key, {}) if isinstance(params.get(model_key), dict) else {}
    model_params = _merge_params(global_params, model_block)

    return router_params, model_params
...
def run_forecast_router(
    ...
    # Router params efectivos (soporta plano o anidado)
    router_params, _ = _resolve_router_and_model_params(model_params, model_key=None)

    # detectar modo desde router_params (si viene)
    router_mode = (router_params.get("router_mode", "") or "").strip().lower()
    ...
    if not seq:
        audit = {
            ...
            "model_params_scope": ("nested" if _is_nested_model_params(model_params) else "flat"),
        }
    ...
    for i, model_key in enumerate(seq, start=1):
        ...
        try:
            ...
            # Params efectivos por modelo (merge global + <model_key>)
            _, runner_params = _resolve_router_and_model_params(model_params, model_key=model_key)
