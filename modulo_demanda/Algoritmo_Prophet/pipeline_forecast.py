...
def merge_model_params(defaults: Dict[str, Any], overrides: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for k, v in overrides.items():
        if isinstance(v, dict) and isinstance(out.get(k), dict):
            ...
            out[k] = v
    return out
def _resolve_path_near_this_file(p: str) -> str:
    """
    Resuelve rutas relativas respecto al directorio de ESTE archivo,
    no respecto al working directory del proceso (Node).
    """
    if not p:
        return p
    p = str(p)
...
def _defaults_path_for_freq(freq: str) -> str:
    """
    Selección automática de defaults por frecuencia efectiva:
      - D     -> model_params.daily.json
      - W-*   -> model_params.weekly.json
      - M*    -> model_params.monthly.json
    """
    f = (freq or "").upper()
    if f.startswith("W"):
        return "model_params.weekly.json"
    if f.startswith("M"):
        return "model_params.monthly.json"
    return "model_params.daily.json"


def _inject_flat_overrides_into_nested(model_params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Compatibilidad: si llegan overrides tipo 'flat' (top-level),
    los inyecta a los bloques anidados ('global'/'router') para que
    el router los vea aun cuando defaults son nested.

    IMPORTANTE:
    - Si el default trae la llave pero vacía ('' / None / {} / []), y en flat viene un valor NO vacío,
      entonces el flat DEBE sobrescribir para no perder el source_collection / recomendaciones.

    Keys que se soportan en flat:
      - router_mode -> global.router_mode
      - dfu_recommendations -> router.dfu_recommendations
      - dfu_selected_models -> router.dfu_selected_models
      - router_source_collection -> router.router_source_collection
      - router_fallback -> router.router_fallback
    """
    if not _is_nested_model_params(model_params):
        return model_params

    def _is_empty(v: Any) -> bool:
        return v is None or v == "" or v == {} or v == []  # simple, suficiente aquí

    # asegura bloques
    global_block = model_params.get("global")
    if not isinstance(global_block, dict):
        global_block = {}
        model_params["global"] = global_block

    router_block = model_params.get("router")
    if not isinstance(router_block, dict):
        router_block = {}
        model_params["router"] = router_block

    # router_mode (vive en global)
    if "router_mode" in model_params:
        v = model_params.get("router_mode")
        if not _is_empty(v) and (_is_empty(global_block.get("router_mode"))):
            global_block["router_mode"] = v

    # dfu_recommendations (vive en router)
    if "dfu_recommendations" in model_params:
        v = model_params.get("dfu_recommendations")
        if not _is_empty(v) and _is_empty(router_block.get("dfu_recommendations")):
            router_block["dfu_recommendations"] = v

    # dfu_selected_models (vive en router)
    if "dfu_selected_models" in model_params:
        v = model_params.get("dfu_selected_models")
        if not _is_empty(v) and _is_empty(router_block.get("dfu_selected_models")):
            router_block["dfu_selected_models"] = v

    # router_source_collection (vive en router)
    if "router_source_collection" in model_params:
        v = model_params.get("router_source_collection")
        if not _is_empty(v) and _is_empty(router_block.get("router_source_collection")):
            router_block["router_source_collection"] = v

    # router_fallback (vive en router)
    if "router_fallback" in model_params:
        v = model_params.get("router_fallback")
        if not _is_empty(v) and _is_empty(router_block.get("router_fallback")):
            router_block["router_fallback"] = v

    # opcional: limpiar llaves flat para evitar confusiones futuras
    for k in ("router_mode","dfu_recommendations","dfu_selected_models","router_source_collection","router_fallback"):
        if k in model_params and k not in ("global","router","arima","prophet","tsb","croston","sba"):
            # mantenemos por compatibilidad si alguien espera leerlos arriba; pero normalmente estorban.
            pass
...
def ejecutar_pipeline(
    ...
    Pipeline universal:
      ...
      - Devuelve:
          df_forecast: (Producto,Canal,Ubicacion,Fecha,Demanda Predicha,forecast_date)
    ...
    # -------------------------------------------------------------
    # -------------------------------------------------------------
    # 0) Guardar overrides (si vienen). Los defaults se eligen después,
    #    cuando ya sepamos la frecuencia efectiva (D / W-MON / MS).
    # -------------------------------------------------------------
    overrides_in = model_params or {}
    ...
    # -------------------------------------------------------------
    # 2.1) Selección automática de defaults por frecuencia
    # -------------------------------------------------------------
    # Si quieres forzar un archivo específico, define:
    #   MODEL_PARAMS_DEFAULTS="ruta/al/archivo.json"
    forced = (os.getenv("MODEL_PARAMS_DEFAULTS", "") or "").strip()
    if forced:
        defaults_src = "env:MODEL_PARAMS_DEFAULTS"
        defaults_path = _resolve_path_near_this_file(forced)
    else:
        defaults_src = "auto_by_freq"
        defaults_path = _resolve_path_near_this_file(_defaults_path_for_freq(freq))

    defaults = load_default_model_params_json(defaults_path)

    # model_params (override) puede venir {}, dict parcial o None
    model_params = merge_model_params(defaults, overrides_in)

    # Compat: si llegan overrides flat (top-level), inyéctalos a nested/global/router
    model_params = _inject_flat_overrides_into_nested(model_params)

    # router_params se calculan a partir de model_params (flat o nested)
    router_params = _resolve_router_params(model_params)

    print(f"[CFG] defaults_src={defaults_src} defaults_path={defaults_path}")
    ...
    for i, (Producto, Canal, Ubicacion) in enumerate(combinaciones_validas, start=1):
        ...
        try:
            ...
            # Horizonte futuro: frontera = último dato real del DFU
            frontera = pd.to_datetime(filtered_df['Fecha'].max())  # frontera por DFU
            ...
            futuro["tipo"] = "future"
            futuro["Demanda Real"] = None
            futuro = futuro[["Producto","Canal","Ubicacion","Fecha","Demanda Real","Demanda Predicha","forecast_date","tipo"]]
            datos_forecast_mongo.append(futuro)
            ...
            try:
                ...
                # -----------------------------
                # Guardar "Predicción Histórica" (fit / in-sample)
                # -----------------------------
                # Esperamos columnas estándar: ds (fecha), y (real), yhat (predicha)
                df_fit = df_merged.copy()
                if "ds" in df_fit.columns:
                    df_fit["Fecha"] = pd.to_datetime(df_fit["ds"])
                elif "Fecha" in df_fit.columns:
                    df_fit["Fecha"] = pd.to_datetime(df_fit["Fecha"])
                else:
                    raise ValueError("df_merged no contiene columna 'ds' ni 'Fecha'.")

                # frontera por DFU (no global): último dato real del DFU
                frontera_dfu = pd.to_datetime(filtered_df["Fecha"].max())

                # quedarnos solo con histórico (<= frontera_dfu) y con real disponible
                if "y" in df_fit.columns:
                    df_fit["Demanda Real"] = df_fit["y"]
                if "yhat" in df_fit.columns:
                    df_fit["Demanda Predicha"] = df_fit["yhat"]

                df_fit = df_fit[df_fit["Fecha"] <= frontera_dfu].copy()
                df_fit = df_fit[df_fit["Demanda Real"].notna()].copy()

                if not df_fit.empty:
                    df_fit["Producto"] = Producto
                    df_fit["Canal"] = Canal
                    df_fit["Ubicacion"] = Ubicacion
                    df_fit["forecast_date"] = forecast_date
                    df_fit["tipo"] = "historical"
                    df_fit = df_fit[[
                        "Producto","Canal","Ubicacion","Fecha",
                        "Demanda Real","Demanda Predicha","forecast_date","tipo"
                    ]]
                    datos_forecast_mongo.append(df_fit)
    ...
    df_forecast = pd.concat(datos_forecast_mongo, ignore_index=True) if datos_forecast_mongo else pd.DataFrame()
    ...
    if not df_forecast.empty:
        print(f"\n[PIPE] Total filas demand_forecast (historical+future): {len(df_forecast)}")
