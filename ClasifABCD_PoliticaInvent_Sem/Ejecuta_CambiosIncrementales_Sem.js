...
const POL_COLL = process.env.POL_COLL_SEM || 'politica_inventarios_01_sem';
const UI_ALL_POL_INV_COLL = process.env.UI_ALL_POL_INV_COLL_SEM || 'ui_sem_all_pol_inv';
...
async function ejecutarIncrementalSolo() {
  ...
  try {
    ...
    // PASO 5: FUSIONAR DATOS DE LOS BACKUPS CON LOS NUEVOS DATOS - POLÍTICAS
    writeToLog(` \n========== FUSIONANDO DATOS DE LOS BACKUPS (POLÍTICAS) ==========`);
    await mergeBackupToPolitica(db, backupNamePolitica);
    await mergeBackupToUiAllPolInv(db, backupNameUiAll);

    // ⭐ PASO 6: FUSIONAR SKUs DEL BACKUP (NUEVO)
    writeToLog(` \n========== FUSIONANDO DATOS DE LOS BACKUPS (SKUs) ==========`);
    await ejecutarArchivo('RestoreSKU_FromBackup_Incremental.js', parametros);

    // PASO 7: ELIMINAR BACKUPS INMEDIATAMENTE DESPUÉS DEL MERGE EXITOSO
