    ...
    const result = await col1
      .aggregate([
        ...
        },
        {
          $set: {
            Lead_Time_Abasto: {
              $toDouble: { $ifNull: ["$joinedData.LeadTime_Abasto_Dias", 0] },
            },
            Fill_Rate: { $toDouble: { $ifNull: ["$joinedData.Fill_Rate", 0] } },
            Frecuencia_Revision_dias: {
              $toInt: { $ifNull: ["$joinedData.Frecuencia_Revision_Dias", 0] },
            },
            MOQ: { $toInt: { $ifNull: ["$joinedData.MOQ", 0] } },
          },
        },
      ])
      .toArray();
