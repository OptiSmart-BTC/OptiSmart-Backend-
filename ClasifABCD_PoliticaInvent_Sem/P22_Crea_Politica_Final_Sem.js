...
async function crearPoliticaInventariosFinal() {
  ...
  try {
    ...
    writeToLog('P22_SEM - Creando tabla final politica_inventarios_01_sem CON HISTORIAL');
    ...
    if (!debeEjecutarse) {
      writeToLog('P22_SEM SALTADO - Ejecución reciente detectada o cambios ya procesados');
      console.log('P22_SEM: Saltado por ejecución reciente');
      return;
    }
    ...
    if (esIncremental) {
      ...
      // PROCESAR SEÑALES DE P24_SEM
      await procesarSenalesP24(db);
      
    } else {
      writeToLog('MODO COMPLETO - Creando politica_inventarios_01_sem completa');
      await crearPoliticaCompleta(db);
      
      // MARCAR ESTADO COMPLETO COMPLETADO
      await marcarEstadoProcesamientoCompletado(db, 'COMPLETO', []);
    }
    writeToLog('P22_SEM completado exitosamente');
  } catch (err) {
    writeToLog(`Error en P22_SEM: ${err.message}`);
    console.error("Error en P22_SEM:", err);
    throw err;
  } finally {
    await client.close();
  }
...
async function verificarSiDebeEjecutarseConRecargas(db) {
  try {
    ...
    if (FORCE_EXECUTION) {
      writeToLog('PARÁMETRO --force detectado - Forzando ejecución P22_SEM');
      return true;
    }
    ...
    if (recargasRecientes.length > 0) {
      ...
      writeToLog(`   FORZANDO EJECUCIÓN P22_SEM para integrar recargas`);
      return true; // Forzar ejecución para recargas
    }
    
    // *** VERIFICAR MÚLTIPLES FUENTES DE ESTADO ***
    
    // 1. VERIFICAR ESTADO DE P22_SEM
    const estadoP22 = db.collection('estado_procesamiento_p22_sem');
    const estadoRecienteP22 = await estadoP22.findOne({
      _id: 'ultimo_procesamiento'
    });
    
    // 2. VERIFICAR ESTADO DE P24_SEM (más específico)
    const estadoP24 = db.collection('estado_procesamiento_p24_sem');
    const estadoRecienteP24 = await estadoP24.findOne({
      _id: 'ultimo_procesamiento_incremental'
    });
    
    // 3. VERIFICAR LOCK TEMPORAL
    const lockCollection = db.collection('p22_execution_lock_sem');
    ...
    if (estadoMasReciente && estadoMasReciente.completado) {
      ...
      if (minutosTranscurridos < ventanaLimite) {
        ...
        if (esIncremental && minutosTranscurridos < (VENTANA_INCREMENTAL_SEGUNDOS / 60)) {
          ...
          writeToLog(`   P22_SEM ya procesó estas ubicaciones hace ${(minutosTranscurridos * 60).toFixed(1)} segundos`);
          return false;
        }
    ...
    if (cambiosRecientes.length > 0) {
      ...
      if (todosProcesadosRecientemente && todosCambios.length > 0) {
        ...
        writeToLog(`   TODOS los cambios fueron procesados recientemente - SALTANDO P22_SEM`);
        return false;
      }
    }
      ...
      if (cambiosPendientes.length > 0) {
        ...
        if (estadoMasReciente.hash_ubicaciones === hashActual) {
          ...
          if (esReciente) {
            ...
            writeToLog(`   EVITANDO DUPLICADO - SALTANDO P22_SEM`);
            return false;
          }
        }
      }
...
async function procesarSenalesP24(db) {
  try {
    ...
    if (senal && senal.ready && !senal.processed_by_p22) {
      ...
      writeToLog(`Señal marcada como procesada por P22_SEM`);
    }
    ...
    writeToLog(`Error procesando señales P24_SEM: ${err.message}`);
  }
}
...
async function marcarEstadoProcesamientoCompletado(db, tipoProcessing, ubicacionesProcesadas) {
  try {
    const estadoCollection = db.collection('estado_procesamiento_p22_sem');
    ...
    const estadoActual = {
      ...
      script_origen: 'P22_SEM',
      sesion_id: `P22_SEM_${Date.now()}`,
...
async function detectarModoIncremental(db) {
  try {
    const collections = await db.listCollections({ name: 'cambios_ubicaciones_temp_sem' }).toArray();
    ...
    if (existeCambios) {
      ...
      writeToLog(`Cambios encontrados en cambios_ubicaciones_temp_sem: ${cambios.length}`);
      writeToLog(`   Hay ubicaciones para procesar: ${hayUbicacionesNuevas}`);
      
      return hayUbicacionesNuevas;
    }
    
    writeToLog('No se encontró colección cambios_ubicaciones_temp_sem - Modo COMPLETO');
    return false;
  } catch (err) {
...
async function getUbicacionesIncrementalesConRecargas(db) {
  try {
    const cambios = await db.collection('cambios_ubicaciones_temp_sem').find({
      $or: [
        { tipo_cambio: 'NUEVA' },
        { tipo_cambio: 'ACTUALIZADA' },
...
async function integrarPoliticaIncrementalConHistorialConRecargas(db, ubicacionesNuevas) {
  writeToLog('P22_SEM - MODO INCREMENTAL: Organizando datos YA procesados por P24_SEM (incluyendo recargas)...');
  ...
  if (recargasEnConjunto.length > 0) {
    writeToLog(`RECARGAS DETECTADAS EN P22_SEM:`);
    ...
    writeToLog(`   Las recargas ya fueron limpiadas por P24_SEM - procediendo con organización normal`);
  }

  const targetCollection = db.collection('politica_inventarios_01_sem');

  // *** VERIFICAR QUE YA EXISTEN DATOS PROCESADOS POR P24_SEM ***
  writeToLog(`Verificando datos procesados por P24_SEM para ubicaciones: [${ubicacionesNuevas.join(', ')}]`);
  
  // Para recargas, solo deberían existir datos nuevos (ya que P24_SEM limpió los anteriores)
  const datosExistentes = await targetCollection.find({
    Ubicacion: { $in: ubicacionesNuevas },
    $or: [
      { Es_Actual: { $exists: false } },  // Datos nuevos de P24_SEM
      { Es_Actual: true },                // Datos ya marcados
      { Es_Actual: false }                // Datos históricos
    ]
  }).toArray();
  
  writeToLog(`Datos encontrados en politica_inventarios_01_sem: ${datosExistentes.length} registros`);

  if (datosExistentes.length === 0) {
    writeToLog('No se encontraron datos procesados por P24_SEM - P22_SEM no puede continuar');
    writeToLog('   P24_SEM debe ejecutarse primero para generar los datos');
    return;
  }
  ...
  if (ubicacionesConRecarga.length > 0) {
    ...
    for (const ubicacionRecargada of ubicacionesConRecarga) {
      ...
      // Marcar todos como actuales (ya que P24_SEM limpió los anteriores)
          ...
          await targetCollection.updateOne(
            { _id: regHistorico._id },
            {
              $set: {
                ...
                Motivo_Historico: 'Duplicado resuelto por P22_SEM'
              }
            }
          );
  ...
  writeToLog(`P22_SEM INCREMENTAL COMPLETADO (con recargas):`);
    ...
    await cambiosCollection.updateMany(
      ...
      },
      {
        $set: {
          ...
          fecha_completado: new Date(),
          procesado_completamente_por: 'P22_SEM'
        }
      }
    );
...
async function crearPoliticaCompleta(db) {
  writeToLog('P22_SEM - MODO COMPLETO: Organizando TODOS los datos YA procesados por P24_SEM...');
  ...
  if (totalRegistros === 0) {
    writeToLog(' No hay datos en politica_inventarios_01_sem - P24_SEM debe ejecutarse primero');
    writeToLog('   P22_SEM solo organiza datos, no los crea. P24_SEM es quien hace los cálculos.');
    return;
  }
  
  writeToLog(` Datos encontrados en politica_inventarios_01_sem: ${totalRegistros} registros`);
  writeToLog('   Procediendo a organizar datos YA calculados por P24_SEM...');
  ...
  const resultadoHistorial = await targetCollection.updateMany(
    { Es_Actual: { $ne: false } },
    {
      $set: {
        ...
        Fecha_Historico: fechaEjecucion,
        Version_Historico: versionEjecucion,
        Motivo_Historico: 'P22_SEM: Reorganización completa - marcando versión anterior como histórica'
      }
    }
  );
      ...
      await targetCollection.updateOne(
        { _id: item.registroMasReciente._id },
        {
          $set: {
            ...
            Procesado_Por: 'P22_SEM_Organizacion',
            Nota_Procesamiento: 'Datos calculados por P24_SEM, organizados por P22_SEM'
          }
        }
      );
      registrosActualizados++;
  ...
  writeToLog(`\n ========== RESUMEN FINAL P22_SEM COMPLETO ==========`);
      ...
      await targetCollection.updateOne(
        { _id: doc._id },
        {
          $set: {
            ...
            Motivo_Historico: 'P22_SEM: Duplicado resuelto - manteniendo más reciente'
          }
        }
      );
      totalLimpiados++;
...
async function generarReporteRecargas(db, ubicacionesProcesadas) {
  try {
    ...
    if (recargasProcesadas.length > 0) {
      ...
      const reporteCollection = db.collection('historial_recargas_completadas_sem');
      const reporte = {
...
async function getNextEjecucionNumber(db) {
  try {
    const contadorCollection = db.collection('contadores_ejecucion_sem');
...
function writeToLog(message) {
  ...
  const logMessage = `[${timestamp}] P22_SEM: ${message}`;
