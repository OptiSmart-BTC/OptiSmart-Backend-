...
async function calcularDiasCobertura() {
  ...
  //writeToLog('------------------------------------------------------------------------------');
  writeToLog(`\nPaso 07 - Transforamación de datos de salida a días de cobertura`);
  ...
  try {
    ...
    const finalCollection = db.collection('ui_sem_pol_inv_dias_cobertura');
    ...
    const diasCobertura = inventarios01.map((inventario) => ({
      ...
      SS: Math.ceil(7*(inventario.SS_Cantidad / inventario.Demanda_Promedio_Semanal)),
      Demanda_LT: Math.ceil(7*(inventario.Demanda_LT / inventario.Demanda_Promedio_Semanal)),
      MOQ: Math.ceil(7*(inventario.MOQ / inventario.Demanda_Promedio_Semanal)),
      ROQ: Math.ceil(7*(inventario.ROQ / inventario.Demanda_Promedio_Semanal)),
      ROP: Math.ceil(7*(inventario.ROP / inventario.Demanda_Promedio_Semanal)),
      META: Math.ceil(7*(inventario.META / inventario.Demanda_Promedio_Semanal)),
      Inventario_Promedio: Math.ceil(7*(inventario.Inventario_Promedio / inventario.Demanda_Promedio_Semanal)),
      Vida_Util_Dias:0,
