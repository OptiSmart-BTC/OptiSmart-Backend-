// P23_Identifica_Cambios_Ubicaciones_Sem.js - VERSIÓN SEMANAL MODIFICADA PARA RECARGA BASADA EN UBIS_SAVED
...
if (process.argv.length < 5) {
  console.error("Uso: node P23_Identifica_Cambios_Ubicaciones_Sem.js <dbName> <DBUser> <DBPassword>");
  process.exit(1);
}
...
async function identificarCambiosConRecargas() {
    console.log('INICIANDO P23_SEM - Identificación de Cambios basada en ubis_saved');
    writeToLog(`P23_SEM - Identificación de Cambios basada en ubis_saved`);
    ...
    try {
        ...
        writeToLog(`P23_SEM COMPLETADO - Total cambios detectados: ${cambiosDetectados.length}`);
        console.log(`P23_SEM completado - ${cambiosDetectados.length} cambios detectados`);
        
    } catch (err) {
        writeToLog(`ERROR en P23_SEM: ${err.message}`);
        console.error('ERROR en P23_SEM:', err);
        throw err;
    } finally {
        if (client) {
            await client.close();
...
async function detectarTiposDeCambios(db, ubicacionesActuales) {
    ...
    // CONSULTAR UBIS_SAVED PARA DETERMINAR TIPO DE CAMBIO (COMPARTIDA ENTRE DIARIO Y SEMANAL)
    ...
    for (const ubicacion of ubicacionesActuales) {
        if (setUbisSaved.has(ubicacion)) {
            ...
            cambios.push({
                ...
                tipo_cambio: 'NUEVA',
                fecha_cambio: new Date(),
                descripcion: 'Ubicación nueva (primera vez detectada) - Procesamiento Semanal',
                es_primera_vez: true,
                procesado: false,
                procesamiento_tipo: 'SEMANAL'
            });
    ...
    writeToLog(`\nRESUMEN DE CAMBIOS DETECTADOS (SEMANAL)`);
    ...
    if (cambios.length === 0) {
        ...
        cambios.push({
            ...
            fecha_cambio: new Date(),
            descripcion: 'No hay ubicaciones nuevas ni recargas detectadas - Procesamiento Semanal',
            procesado: false,
            procesamiento_tipo: 'SEMANAL'
        });
    }
...
async function analizarRecargaUbicacion(db, ubicacion) {
    try {
        ...
        if (!ubicacionHistorica) {
            return {
                razon: 'Recarga detectada (Semanal)',
        ...
        let razon = 'Recarga de ubicación existente (Semanal)';
        let detalles = '';
        
        if (!estabaActiva) {
            razon = 'Reactivación de ubicación inactiva (Semanal)';
            detalles = `Ubicación estaba inactiva, ahora tiene ${skusActuales} SKUs`;
        } else if (Math.abs(skusActuales - registrosAnteriores) > 0) {
            razon = 'Recarga con cambios en datos (Semanal)';
            ...
            razon = 'Recarga de ubicación activa (Semanal)';
...
async function crearTablaCambios(db, cambios) {
    ...
    writeToLog(`Tabla cambios_ubicaciones_temp_sem limpiada`);
    ...
    if (cambios.length === 0) {
        ...
        await cambiosCollection.insertOne({
            tipo_cambio: 'NO_CAMBIOS',
            fecha_cambio: new Date(),
            descripcion: 'No se detectaron cambios en las ubicaciones - Procesamiento Semanal',
            procesado: false,
            procesamiento_tipo: 'SEMANAL'
        });
    ...
    await cambiosCollection.insertMany(cambios.map(cambio => ({
        ...
        id_sesion: `P23_SEM_${moment().format('YYYYMMDD_HHmmss')}`,
        procesamiento_tipo: 'SEMANAL'
    })));
    
    writeToLog(`Tabla cambios_ubicaciones_temp_sem creada con ${cambios.length} registros`);
...
async function actualizarUbicacionesGuardadas(db, ubicacionesActuales) {
    const ubicacionesCollection = db.collection('ubicaciones_guardadas_sem');
    ...
    const documento = {
        ...
        version_script: 'P23_SEM_v3.0_recarga_por_ubis_saved',
        nota: 'Las ubicaciones se clasifican como NUEVA o RECARGA basándose en ubis_saved - Procesamiento Semanal',
        procesamiento_tipo: 'SEMANAL'
    };
    ...
    writeToLog(`Nota: El historial completo se mantiene en 'ubis_saved' (compartido con diario)`);
}

function writeToLog(message) {
    const timestamp = moment().format('YYYY-MM-DD HH:mm:ss');
    const logMessage = `[${timestamp}] P23_SEM: ${message}`;
...
if (require.main === module) {
    identificarCambiosConRecargas()
        .then(() => {
            writeToLog('P23_SEM completado exitosamente');
            process.exit(0);
        })
        .catch((err) => {
            writeToLog(`P23_SEM falló: ${err.message}`);
            console.error(err);
            process.exit(1);
        });
}