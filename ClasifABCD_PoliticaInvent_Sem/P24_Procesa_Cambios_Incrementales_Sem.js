...
async function procesaCambiosIncrementales() {
    console.log('INICIANDO P24 - Procesamiento Incremental de Ubicaciones SEM');
    writeToLog(`P24 - Procesamiento Incremental de Ubicaciones SEM`);
    ...
    try {
        ...
        if (ubicacionesAProcesar.length > 0) {
            ...
            try {
                ...
                // PASO 6: CREAR TABLA FINAL politica_inventarios_01_sem
                writeToLog(`CREANDO TABLA FINAL politica_inventarios_01_sem...`);
...
async function verificarSiYaSeEjecutoConRecargas(db) {
    try {
        ...
        const estadoCollection = db.collection('estado_procesamiento_p24_sem');
...
async function prepararDatosIncrementales(db, ubicacionesAProcesar, skusEnUbicaciones) {
    ...
    await db.collection('sku_backup_incremental_sem').deleteMany({});
    if (todosSkusOriginales.length > 0) {
        await db.collection('sku_backup_incremental_sem').insertMany(todosSkusOriginales);
...
async function restaurarSkusOriginales(db) {
    ...
    try {
        const skuCollection = db.collection('sku');
        const backupCollection = db.collection('sku_backup_incremental_sem');
        
        const collections = await db.listCollections({ name: 'sku_backup_incremental_sem' }).toArray();
        if (collections.length === 0) {
...
async function marcarCambiosComoProcesadosConCoordinacionConRecargas(db, ubicacionesProcesadas) {
    ...
    try {
        ...
        if (cambiosPendientesP22.length > 0) {
            ...
            await db.collection('p22_signals_sem').replaceOne(
                { _id: 'incremental_ready' },
                signalData,
                { upsert: true }
            );
...
async function verificarYPrepararP22(db, ubicacionesProcesadas) {
    try {
        ...
        // 1. VERIFICAR QUE ui_sem_all_pol_inv TIENE DATOS
        const uiAllPolInv = db.collection('ui_sem_all_pol_inv');
        const datosUI = await uiAllPolInv.find({
            Ubicacion: { $in: ubicacionesProcesadas }
        }).toArray();
        
        if (datosUI.length === 0) {
            writeToLog(`ADVERTENCIA: No hay datos en ui_sem_all_pol_inv para ubicaciones procesadas`);
            writeToLog(`   P22 no podrá integrar - verificar scripts de políticas`);
            return;
        }
        
        writeToLog(`ui_sem_all_pol_inv tiene ${datosUI.length} registros para integración`);
        
        // 2. CREAR METADATA PARA P22
        await db.collection('p22_integration_metadata_sem').replaceOne(
            { _id: 'incremental_ready' },
            {
                _id: 'incremental_ready',
...
async function verificarP22CompletadoParaUbicaciones(db, ubicaciones) {
    try {
        const estadoP22 = await db.collection('estado_procesamiento_p22_sem').findOne({
...
async function finalizarP24ConVerificacion(db, ubicacionesAProcesar) {
    try {
        ...
        await db.collection('estado_procesamiento_p24_sem').updateOne(
            { _id: 'procesamiento_en_curso' },
            {
                $set: {
                    estado: 'COMPLETADO',
        ...
        if (p22Completado) {
            ...
            await db.collection('p22_signals_sem').deleteOne({ _id: 'incremental_ready' });
            await db.collection('p22_execution_lock_sem').deleteOne({ _id: 'p22_incremental_lock' });
            await db.collection('p22_integration_metadata_sem').deleteOne({ _id: 'incremental_ready' });
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (!p22EjecutadoDespues) {
                        ...
                        await dbLimpieza.collection('p22_signals_sem').deleteOne({ _id: 'incremental_ready' });
                        await dbLimpieza.collection('p22_execution_lock_sem').deleteOne({ _id: 'p22_incremental_lock' });
                        await dbLimpieza.collection('p22_integration_metadata_sem').deleteOne({ _id: 'incremental_ready' });
...
function writeToLog(message) {
    ...
    const logMessage = `[${timestamp}] P24_SEM: ${message}`;
