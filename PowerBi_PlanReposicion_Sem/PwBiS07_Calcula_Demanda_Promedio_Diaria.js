...
async function actualizarDatos() {
  //writeToLog('------------------------------------------------------------------------------');
  writeToLog(`\nPaso 03 - Calculo de la Demanda Promedio Semanal`);
    ...
    },
    {
      $addFields: {
        Demanda_Promedio_Semanal: {
          $ifNull: [{ $arrayElemAt: ['$inventario_info.Demanda_Promedio_Semanal', 0] }, "SIN DEMANDA"]
        }
      }
    },
    {
  ...
  for (const doc of result) {
    ...
    await col1.updateOne(
      ...
      {
        $set: {
          Demanda_Promedio_Semanal: doc.Demanda_Promedio_Semanal
        }
      }
    );
  }
  ...
  writeToLog(`\tTermina el Calculo de la Demanda Promedio Semanal`);
  } catch (error) {
    // Manejar el error
