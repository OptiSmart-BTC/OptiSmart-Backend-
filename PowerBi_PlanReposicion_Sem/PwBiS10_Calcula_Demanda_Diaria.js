...
async function actualizarDatos() {
  ...
  //writeToLog('------------------------------------------------------------------------------');
  writeToLog(`\nPaso 10 - Calcula la Demanda Diaria`);
  ...
  const result = await col.aggregate([
    {
      $project: {
        ...
        Demanda_Diaria: {
          $cond: [
            { $or: [{ $eq: ["$Demanda_Promedio_Semanal", "SIN DEMANDA"] }, { $eq: ["$Demanda_Promedio_Semanal", null] }] },
            0, 
            { $divide: [{ $toDouble: "$Demanda_Promedio_Semanal" }, 7] }
          ]
        },
      }
    }
...
console.log(result);
  writeToLog(`\tTermina el Calculo de la Demanda Diaria`);
  } catch (error) {
