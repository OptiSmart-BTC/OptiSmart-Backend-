  ...
  const result = await col1.aggregate([
    ...
    },
    {
      $set: {
        ...
        //'Override_SI_NO': { $cond: [{ $ne: ['$joinedData.OverrideSafetyStock_Cantidad', 0] }, 'SI', 'NO'] },
        'Override_SI_NO': {
          $cond: {
            if: {
              $in: [
                '$joinedData.OverrideSafetyStock_UOM_Base',
                ["0", ' ', null, ""]
              ]
            },
            then: 'NO',
          ...
          /*$cond: [
              {
                $or: [
                  { $ne: ['$joinedData.OverrideSafetyStock_Cantidad', '0'] },
                  { $ne: ['$joinedData.OverrideSafetyStock_Cantidad', ' '] },
                  { $eq: ['$joinedData.OverrideSafetyStock_Cantidad', null] }
                ]
              },
              'SI',
              'NO'
