...
async function main() {
  ...
  writeToLog(`\nPaso 06.0 - Se obtienen los SKU que tienen Override, Tienen Historia pero no estan ene el rango de fechas`);
  ...
  try {
    ...
    const clasificacionCollection = database.collection('demanda_ordenada_desc');
    ...
    for (const sku of skus) {
      ...
      if ((Override_Min_Politica_Inventarios !== null && Override_Min_Politica_Inventarios !== '') || (Override_Max_Politica_Inventarios !== null && Override_Max_Politica_Inventarios !== '')) {
        ...
        if (historia) {
          const clasificacion = await clasificacionCollection.findOne({ Producto, Ubicacion });
