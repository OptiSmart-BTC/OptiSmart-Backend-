...
async function realizarAgregacion_Demanda_UOM() {
  ...
  writeToLog(`\nPaso 1 - Calculo de la Demanda Costos`);
...
//--------------------------------------------------------------------------------------------
    ...
    const ColeccionRespaldo = db.collection('demanda_calculada');

    const skuDocs = await historicoDemandaCollection.find({}).toArray();
    const skus = skuDocs.map((doc) => doc.SKU);
    // Obtener los registros del historial de demanda que no tienen SKU en la colección "sku"
    /*const skusNoEncontrados = await skuCollection
      .find({ SKU: { $nin: skus } })
      .toArray();*/
    ...
    /*  const skusNoEncontrados = await skuCollection
      ...
      .project({ Producto: 1, Ubicacion: 1, _id: 0 }) 
      .toArray();
*/
...
const skusNoEncontrados = await skuCollection
  .aggregate([
    {
      $match: {
        SKU: { $nin: skus } // SKUs que no están en la lista skus
      }
    },
    {
      $addFields: {
        Demanda_Costo: 0 // Agrega el campo Demanda_Costo con el valor predeterminado de 0
      }
    },
    {
      $project: {
  ...
  await ColeccionRespaldo.insertMany(skusNoEncontrados);

       //writeToLog(JSON.stringify(skusNoEncontrados, null, 2));
    ...
    writeToLog(`\tTermina el calculo de la Demanda en Costos`);
  } catch (error) {
