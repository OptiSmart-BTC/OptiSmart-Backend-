...
const demandaAbcd01Collection = 'demanda_abcd_01'; 

async function calcularPromedioErrorCuadrado() {

  const now = moment().format('YYYY-MM-DD HH:mm:ss');

  writeToLog(`\nPaso 08 - Calculo de la Variabilidad de la Demanda`);
  ...
  try {
    ...
    const resultadosAgregados = await db.collection(historicoDemandaCollection).aggregate([
      ...
      },
     {
        $group: {
          ...
          Demanda_Costo: { $sum: "$Error_Cuadrado" }
        }
      }, 
      {
        $addFields: {
    ...
    const demandaAbcd01Collection = db.collection('demanda_abcd_01'); // Reemplaza con el nombre de tu colección
    ...
    //const productos = resultadosDivididos.map(resultado => resultado.Producto);
    //const ubicaciones = resultadosDivididos.map(resultado => resultado.Ubicacion);
