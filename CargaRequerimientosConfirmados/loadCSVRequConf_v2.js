...
const csvFilePath = `../../${parametroFolder}/csv/in/requerimientos_confirmados.csv`; // Cambia esta ruta según la ubicación de tu archivo CSV
  ...
  try {
    ...
    fs.createReadStream(csvFilePath)
      ...
      .on('end', async () => {
        ...
        let numRegistrosCargados;
        // Verificar si hay datos para insertar
        if (results.length > 0) {
          await collection.insertMany(results);
          //writeToLog(`${results.length}`);
          numRegistrosCargados = results.length;
        } ;
