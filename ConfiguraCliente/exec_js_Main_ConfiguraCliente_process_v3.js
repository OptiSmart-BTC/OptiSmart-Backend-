...
const {
  ...
  path_opti,
  path_users,
} = require(`../Configuraciones/paths_vars`);
...
async function obtenerDBNamePorAppUser(appUser) {
  ...
  try {
    ...
    if (resultado && resultado.UserUI) {
      userInformation = resultado.UserUI.find(
      ...
      if (userInformation) {
        ...
        console.log(
          `No se encontró el subdocumento para el AppUser: ${appUser}`
        );
      }
    } else {
    ...
    const archivos = [
      ...
      //${var_PwBiUser} ${var_PwBiPassword} ${var_PwBiURL}
      {
