...
const archivos = [
  ...
  { nombre: 'ValidaCSV_SKU_TipodeDato_v5.js', parametros: `${parametroFolder}` },
  { nombre: 'loadCSVsku_v6.js', parametros: `${dbName} ${parametroFolder}` },
  { nombre: 'Mueve_SKUcsvProcesados.js', parametros: `${parametroFolder}` },
  { nombre: 'LimpiaEspaciosSKU_v3.js', parametros: `${dbName} ${parametroFolder}` }
];
...
function ejecutarArchivos(archivos) {
  ...
  if (archivo.nombre === 'ValidaCSV_Headers_v4.js') {
...
function IniciaejecutarArchivos() {
  ...
  if (fs.existsSync(filePath)) {
    ...
    writeToLog(`Inicia Validaciones y Carga de datos de SKU\n`);
    ejecutarArchivos(archivos);
  } else {
