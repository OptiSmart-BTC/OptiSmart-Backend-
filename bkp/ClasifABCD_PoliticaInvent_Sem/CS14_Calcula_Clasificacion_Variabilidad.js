...
async function updateClasificacionVariabilidad() {
  ...
  writeToLog(`\nPaso 14 - Calculo de la Clasificacion de la Variabilidad por Semana`);
...
//const url = `mongodb://${DBUser}:${DBPassword}@${host}:${puerto}/${dbName}?authSource=admin`;
  ...
  const demandaABCDData = await db.collection('demanda_abcd_01_sem').find().toArray();
  ...
  writeToLog(`\tTermina el Calculo de la Clasificacion de la Variabilidad por Semana`);
