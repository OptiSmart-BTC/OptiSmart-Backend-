...
const collection1 = 'ui_sem_pol_inv_dias_cobertura';
...
async function actualizarDatos01() {
  ...
  try {
    ...
    // Obtener la colección 1 (ui_pol_inv_dias_cobertura)
    ...
    for (const doc of result) {
      ...
      // Usar Override_ROP si está disponible, de lo contrario usar ROP
      const ROP_Utilizado = doc.Override_ROP || doc.ROP;

      const ROP_Alto = Tolerancia_Vida_Util_Dias < ROP_Utilizado ? 'SI' : 'NO';
      const SobreInventario_Dias = Math.max(0, ROP_Utilizado - Tolerancia_Vida_Util_Dias);
    ...
    writeToLog(`\tTermina el proceso de obtención de campos relacionados con el SKU`);
  } catch (error) {
