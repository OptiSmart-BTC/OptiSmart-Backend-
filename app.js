...
const {
  ...
  useradmin,
  passadmin,
} = require("./Configuraciones/ConexionDB");
const app = express();
app.use(cors());
const port = 3000;
...
const rutaDirClasifABCD = path.join(
  directorioActual,
  "ClasifABCD_PoliticaInvent"
);
const rutaDirClasifABCDSem = path.join(
  directorioActual,
  "ClasifABCD_PoliticaInvent_Sem"
);
const rutaDirCargaInvDisponible = path.join(
  directorioActual,
  "CargaInvDisponible"
);
const rutaDirCargaInvTrans = path.join(directorioActual, "CargaInvTrans");
const rutaDirCargaRequerimientosConfirmados = path.join(
  directorioActual,
  "CargaRequerimientosConfirmados"
);
const rutaDirPlanReposicion = path.join(directorioActual, "PlanReposicion");
const rutaDirPlanReposicion_Sem = path.join(
  directorioActual,
  "PlanReposicion_Sem"
);
...
const rutaDirOverridePlanReposicion = path.join(
  directorioActual,
  "OverridePlanReposicion"
);
const rutaDirOverridePlanReposicion_Sem = path.join(
  directorioActual,
  "OverridePlanReposicion_Sem"
);
const rutaDirPoliticaInventarios = path.join(
  directorioActual,
  "OverridePoliticaInventarios"
);
const rutaDirPoliticaInventarios_Sem = path.join(
  directorioActual,
  "OverridePoliticaInventarios_Sem"
);

const rutaDirPowerBi_PlanReposicion = path.join(
  directorioActual,
  "PowerBi_PlanReposicion"
);
const rutaDirPowerBi_PlanReposicion_Sem = path.join(
  directorioActual,
  "PowerBi_PlanReposicion_Sem"
);

const rutaDirMontecarlo = path.join(directorioActual, "montecarlo");
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const user = await dbUsuarios.findOne({
      $or: [{ AppUser: logUsername }, { "UserUI.AppUser": logUsername }],
    });
    ...
    if (user.AppUser === logUsername) {
      ...
      const userDB =
        user.UserDB.find((db) => db.DBName === logUsername) || user.UserDB[0];
    ...
    try {
      ...
      console.error("No se pudo preparar la carpeta del usuario:", error.message);
    }
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    const users = client.db(adminDbName).collection(usersCollectionName);
    const account = await users.findOne({
      $or: [{ AppUser: username }, { "UserUI.AppUser": username }],
    });
    ...
    const isPrimary = account.AppUser === username;
    const user = isPrimary
      ? account
      : account.UserUI?.find((candidate) => candidate.AppUser === username);
    const userDb = isPrimary
      ? account.UserDB?.[0]
      : account.UserDB?.find((candidate) => candidate.DBName === username) ||
        account.UserDB?.[0];
    ...
    const decryptedPassword = await getDecryptedPassUser(user.AppPassword);
    if (password !== decryptedPassword) {
      return res.status(401).json({ error: "Usuario o contraseña incorrectos." });
    }

    const dbName = userDb?.DBName || username;
    conex.setUserData(username, decryptedPassword, `btc_opti_${dbName}`);

    const command = `cd "${rutaDirConfiguraCliente}" && node exec_js_Main_ConfiguraCliente_process_v3.js ${username}`;
    try {
      exec(command, (error) => {
        if (error) {
          console.error(
            "No se pudo preparar la carpeta del usuario:",
            error.message
          );
        }
      });
    ...
    return res.status(200).json({
      ...
      user: {
        ...
        type: user.Type || "A",
        rol: user.rol || "admin",
        empresaId: String(account.EmpresaId || dbName),
        empresaNombre: user.CompanyName || account.CompanyName || dbName,
      },
    });
  } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const rutaDirLogFile = path.join(
        __dirname,
        `../${DBName}/log/LogdeCargaCSV.log`
      );
      ...
      try {
        ...
        fs.access(filePath, fs.constants.F_OK, (err) => {
          if (err) {
            ...
            fs.writeFile(filePath, "", (err) => {
              if (err) {
                console.error(
                  "Error al crear el archivo LogdeCargaCSV.log:",
                  err
                );
                res
                  .status(500)
                  .send("Error al crear el archivo LogdeCargaCSV.log");
                return;
              }
        ...
        function leerArchivoLog(filePath) {
          const logContent = fs.readFile(
            filePath,
            "utf-8",
            (err, logContent) => {
              if (err) {
                console.error(
                  "Error al leer el archivo LogdeCargaCSV.log:",
                  err
                );
                res
                  .status(500)
                  .send("Error al leer el archivo LogdeCargaCSV.log");
                return;
              }
              res.send(logContent);
            }
  ...
  try {
    ...
    if (skuFiles.length > 0) {
      filesToDownload.push({
        name: skuFiles[0].name,
        path: path.join(processedDir, skuFiles[0].name),
      });
    }
    if (historicoFiles.length > 0) {
      filesToDownload.push({
        name: historicoFiles[0].name,
        path: path.join(processedDir, historicoFiles[0].name),
      });
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      fs.renameSync(
        historicoDemandaCsv.path,
        windowsDir + "in/historico_demanda.csv"
      );
...
app.post("/showParams", async (req, res) => {
  try {
    ...
    //----------------------------Variabilidad-----------------------------------
    const parametrosVariabilidadAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
    ...
    const parametrosVariabilidadMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
        Criterio_Clasificacion: "Variabilidad",
    ...
    const parametrosVariabilidadBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
        Criterio_Clasificacion: "Variabilidad",
    ...
    //AltaBajoMuy_Baja
    const parametrosClasAltaBajoMuy_Baja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaBajoMuy Baja",
      });
    const valorClasAltaBajoMuy_Baja =
      parametrosClasAltaBajoMuy_Baja?.Clasificacion_ABCD ?? "N/A";

    //AltaAltoMuy_Baja
    const parametrosClasAltaAltoMuy_Baja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaAltoMuy Baja",
      });
    const valorClasAltaAltoMuy_Baja =
      parametrosClasAltaAltoMuy_Baja?.Clasificacion_ABCD ?? "N/A";

    //AltaBajoBaja
    const parametrosClasAltaBajoBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaBajoBaja",
      });
    const valorClasAltaBajoBaja =
      parametrosClasAltaBajoBaja?.Clasificacion_ABCD ?? "N/A";

    //AltaAltoBaja
    const parametrosClasAltaAltoBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaAltoBaja",
      });
    const valorClasAltaAltoBaja =
      parametrosClasAltaAltoBaja?.Clasificacion_ABCD ?? "N/A";

    //AltaBajoMedia
    const parametrosClasAltaBajoMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaBajoMedia",
      });
    const valorClasAltaBajoMedia =
      parametrosClasAltaBajoMedia?.Clasificacion_ABCD ?? "N/A";

    //AltaAltoMedia
    const parametrosClasAltaAltoMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaAltoMedia",
      });
    const valorClasAltaAltoMedia =
      parametrosClasAltaAltoMedia?.Clasificacion_ABCD ?? "N/A";

    //AltaBajoAlta
    const parametrosClasAltaBajoAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaBajoAlta",
      });
    const valorClasAltaBajoAlta =
      parametrosClasAltaBajoAlta?.Clasificacion_ABCD ?? "N/A";

    //AltaAltoAlta
    const parametrosClasAltaAltoAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "AltaAltoAlta",
      });
    const valorClasAltaAltoAlta =
      parametrosClasAltaAltoAlta?.Clasificacion_ABCD ?? "N/A";

    //MediaBajoMuy_Baja
    const parametrosClasMediaBajoMuy_Baja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaBajoMuy Baja",
      });
    const valorClasMediaBajoMuy_Baja =
      parametrosClasMediaBajoMuy_Baja?.Clasificacion_ABCD ?? "N/A";

    //MediaAltoMuy_Baja
    const parametrosClasMediaAltoMuy_Baja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaAltoMuy Baja",
      });
    const valorClasMediaAltoMuy_Baja =
      parametrosClasMediaAltoMuy_Baja?.Clasificacion_ABCD ?? "N/A";

    //MediaBajoBaja
    const parametrosClasMediaBajoBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaBajoBaja",
      });
    const valorClasMediaBajoBaja =
      parametrosClasMediaBajoBaja?.Clasificacion_ABCD ?? "N/A";

    //MediaAltoBaja
    const parametrosClasMediaAltoBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaAltoBaja",
      });
    const valorClasMediaAltoBaja =
      parametrosClasMediaAltoBaja?.Clasificacion_ABCD ?? "N/A";

    //MediaBajoMedia
    const parametrosClasMediaBajoMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaBajoMedia",
      });
    const valorClasMediaBajoMedia =
      parametrosClasMediaBajoMedia?.Clasificacion_ABCD ?? "N/A";

    //MediaAltoMedia
    const parametrosClasMediaAltoMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaAltoMedia",
      });
    const valorClasMediaAltoMedia =
      parametrosClasMediaAltoMedia?.Clasificacion_ABCD ?? "N/A";

    //MediaBajoAlta
    const parametrosClasMediaBajoAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaBajoAlta",
      });
    const valorClasMediaBajoAlta =
      parametrosClasMediaBajoAlta?.Clasificacion_ABCD ?? "N/A";

    //MediaAltoAlta
    const parametrosClasMediaAltoAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "MediaAltoAlta",
      });
    const valorClasMediaAltoAlta =
      parametrosClasMediaAltoAlta?.Clasificacion_ABCD ?? "N/A";

    //BajaBajoMuy_Baja
    const parametrosClasBajaBajoMuy_Baja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaBajoMuy Baja",
      });
    const valorClasBajaBajoMuy_Baja =
      parametrosClasBajaBajoMuy_Baja?.Clasificacion_ABCD ?? "N/A";

    //BajaAltoMuy_Baja
    const parametrosClasBajaAltoMuy_Baja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaAltoMuy Baja",
      });
    const valorClasBajaAltoMuy_Baja =
      parametrosClasBajaAltoMuy_Baja?.Clasificacion_ABCD ?? "N/A";

    //BajaBajoBaja
    const parametrosClasBajaBajoBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaBajoBaja",
      });
    const valorClasBajaBajoBaja =
      parametrosClasBajaBajoBaja?.Clasificacion_ABCD ?? "N/A";

    //BajaAltoBaja
    const parametrosClasBajaAltoBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaAltoBaja",
      });
    const valorClasBajaAltoBaja =
      parametrosClasBajaAltoBaja?.Clasificacion_ABCD ?? "N/A";

    //BajaBajoMedia
    const parametrosClasBajaBajoMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaBajoMedia",
      });
    const valorClasBajaBajoMedia =
      parametrosClasBajaBajoMedia?.Clasificacion_ABCD ?? "N/A";

    //BajaAltoMedia
    const parametrosClasBajaAltoMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaAltoMedia",
      });
    const valorClasBajaAltoMedia =
      parametrosClasBajaAltoMedia?.Clasificacion_ABCD ?? "N/A";

    //BajaBajoAlta
    const parametrosClasBajaBajoAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaBajoAlta",
      });
    const valorClasBajaBajoAlta =
      parametrosClasBajaBajoAlta?.Clasificacion_ABCD ?? "N/A";

    //BajaAltoAlta
    const parametrosClasBajaAltoAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Variabilidad",
        ID: "BajaAltoAlta",
      });
    const valorClasBajaAltoAlta =
      parametrosClasBajaAltoAlta?.Clasificacion_ABCD ?? "N/A";

    //---------Margen--------------
    //MargenBajo
    const parametrosMargenBajo = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
    ...
    //MargenAlto
    const parametrosMargenAlto = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
        Criterio_Clasificacion: "Margen",
    ...
    //---------------------Demanda------------------------------------------------
    //DemandaMuyBaja
    const parametrosDemandaMuyBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
    ...
    //DemandaBaja
    const parametrosDemandaBaja = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
        Criterio_Clasificacion: "Demanda",
    ...
    //DemandaMedia
    const parametrosDemandaMedia = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
        Criterio_Clasificacion: "Demanda",
    ...
    //DemandaAlta
    const parametrosDemandaAlta = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Criterios",
        Criterio_Clasificacion: "Demanda",
    ...
    //HorizonteHistorico
    const parametrosHorizonteHistorico = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Horizontes",
        Num_Param: 1,
      });
    const valorHorizonteHistorico =
      parametrosHorizonteHistorico?.Horizonte_Historico_dias ?? "N/A";

    //FinHorizonte
    const parametrosFinHorizonte = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Horizontes",
        Num_Param: 2,
    ...
    //NivelServ_A
    const parametrosNivelServ_A = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Nivel_Servicio",
        Clasificacion: "A",
    ...
    //NivelServ_B
    const parametrosNivelServ_B = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Nivel_Servicio",
        Clasificacion: "B",
    ...
    //NivelServ_C
    const parametrosNivelServ_C = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Nivel_Servicio",
        Clasificacion: "C",
    ...
    //NivelServ_D
    const parametrosNivelServ_D = await db
      .collection("parametros_usuario")
      .findOne({
        Tipo: "Nivel_Servicio",
        Clasificacion: "D",
...
app.post("/saveParams", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("Error al guardar los nuevos valores en la base de datos.");
  }
});
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
app.post("/runProcess", async (req, res) => {
  try {
    const { appUser, appPass, DBName, useMontecarlo } = req.body;

    // Pass encriptado (llega desde el frontend)
    const encryptedAppPass = appPass;

    // Pass desencriptado (para conectar a Mongo)
    const decryptedAppPass = await getDecryptedPassUser(encryptedAppPass);
    ...
    if (useMontecarlo) {
      console.log(
        "[RUN Diario] Preparando SS via Montecarlo (solo slow-movers)..."
      );
      await prepararSSAntesDePolitica({
        appUser,
        appPass: encryptedAppPass, // 👈 Montecarlo necesita el ENCRIPTADO
        DBName,
        tipoProceso: "Diario",
        conex,
      });
    ...
    console.log("[RUN Diario] Ejecutando políticas...");
    try {
      const out = execFileSync(
        "node",
        ["exec_js_Main_ClasABCD_PolInvent_process_v2.js", usuarioLog],
        {
          cwd: rutaDirClasifABCD,
          stdio: "pipe",
        }
      );
      console.log("Políticas ejecutadas (Diario):", out.toString());
      res.status(200).json({
        message: useMontecarlo
          ? "Diario: SS (slow-movers) inyectado y políticas ejecutadas."
          : "Diario: Políticas ejecutadas.",
      });
    } catch (error) {
      console.error("Error en execFileSync políticas Diario:");
      console.error("STDOUT:", error.stdout?.toString());
      console.error("STDERR:", error.stderr?.toString());
      return res.status(500).send("Error al ejecutar políticas Diario");
    }
  } catch (err) {
    console.error("Error en /runProcess:", err);
    res.status(500).send("Error al procesar Diario");
  }
});
...
app.post("/runProcessSem", async (req, res) => {
  try {
    ...
    if (useMontecarlo) {
      console.log(
        "[RUN Semanal] Preparando SS via Montecarlo (solo slow-movers)..."
      );
      await prepararSSAntesDePolitica({
        appUser,
        appPass: encryptedAppPass, // 👈 Montecarlo necesita el ENCRIPTADO
    ...
    console.log("[RUN Semanal] Ejecutando políticas...");
    try {
      const out = execFileSync(
        "node",
        ["exec_js_Main_Sem_ClasABCD_PolInvent_process.js", usuarioLog],
        {
          cwd: rutaDirClasifABCDSem,
          stdio: "pipe",
        }
      );
      console.log("Políticas ejecutadas (Semanal):", out.toString());
      res.status(200).json({
        message: useMontecarlo
          ? "Semanal: SS (slow-movers) inyectado y políticas ejecutadas."
          : "Semanal: Políticas ejecutadas.",
      });
    } catch (error) {
      console.error("Error en execFileSync políticas Semanal:");
      console.error("STDOUT:", error.stdout?.toString());
      console.error("STDERR:", error.stderr?.toString());
      return res.status(500).send("Error al ejecutar políticas Semanal");
    }
  } catch (err) {
    console.error("Error en /runProcessSem:", err);
    res.status(500).send("Error al procesar Semanal");
  }
});
...
app.post("/getCSVPol", async (req, res) => {
  ...
  try {
    ...
    if (type === "clasifABCD") {
      collectionName =
        cal === "Diario" ? "ui_demanda_abcd" : "ui_sem_demanda_abcd";
    } else if (type === "polInv") {
      collectionName =
        cal === "Diario" ? "ui_all_pol_inv" : "ui_sem_all_pol_inv";
    }
...
app.post("/getCSVPol1", async (req, res) => {
  ...
  try {
    ...
    if (!data.length) {
      return res
        .status(404)
        .json({ error: "No se encontraron datos en la colección." });
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      fs.renameSync(
        invDispCsv.path,
        windowsDir + "in/inventario_disponible.csv"
      );

      const rutaDirLogFile = path.join(
        __dirname,
        `../${DBName}/log/LogdeCargaInvDispoCSV.log`
      );
      ...
      try {
        ...
        function leerArchivoLog(filePath) {
          fs.readFile(filePath, "utf-8", (err, logContent) => {
            if (err) {
              console.error(
                "Error al leer el archivo LogdeCargaInvDisp.log:",
                err
              );
              res
                .status(500)
                .send("Error al leer el archivo LogdeCargaInvDisp.log");
              return;
            }
            res.send(logContent);
          });
...
app.post("/getActualCSVInvDisp", async (req, res) => {
  try {
    ...
    if (!fs.existsSync(processedDir)) {
      return res
        .status(404)
        .send("El directorio de archivos procesados no existe.");
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      fs.renameSync(
        invTransCsv.path,
        windowsDir + "in/inventario_transito.csv"
      );

      const rutaDirLogFile = path.join(
        __dirname,
        `../${DBName}/log/LogdeCargaInvTransCSV.log`
      );
        ...
        fs.access(filePath, fs.constants.F_OK, (err) => {
          if (err) {
            fs.writeFile(filePath, "", (err) => {
              if (err) {
                console.error(
                  "Error al crear el archivo LogdeCargaInvTransCSV.log:",
                  err
                );
                res
                  .status(500)
                  .send("Error al crear el archivo LogdeCargaInvTransCSV.log");
                return;
              }
              leerArchivoLog(filePath);
            });
        ...
        function leerArchivoLog(filePath) {
          const logContent = fs.readFile(
            ...
            (err, logContent) => {
              if (err) {
                console.error(
                  "Error al leer el archivo LogdeCargaInvTransCSV.log:",
                  err
                );
                res
                  .status(500)
                  .send("Error al leer el archivo LogdeCargaInvTransCSV.log");
                return;
              }
              res.send(logContent);
            }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      fs.renameSync(
        reqConfCsv.path,
        windowsDir + "in/requerimientos_confirmados.csv"
      );

      const rutaDirLogFile = path.join(
        __dirname,
        `../${DBName}/log/LogdeCargaRequConfCSV.log`
      );
        ...
        fs.access(filePath, fs.constants.F_OK, (err) => {
          if (err) {
            fs.writeFile(filePath, "", (err) => {
              if (err) {
                console.error(
                  "Error al crear el archivo LogdeCargaReqConf.log:",
                  err
                );
                res
                  .status(500)
                  .send("Error al crear el archivo LogdeCargaReqConf.log");
                return;
              }
              leerArchivoLog(filePath);
            });
        ...
        function leerArchivoLog(filePath) {
          const logContent = fs.readFile(
            ...
            (err, logContent) => {
              if (err) {
                console.error(
                  "Error al leer el archivo LogdeCargaReqConf.log:",
                  err
                );
                res
                  .status(500)
                  .send("Error al leer el archivo LogdeCargaReqConf.log");
                return;
              }
              res.send(logContent);
            }
    ...
    exec(comandoPlanReposicion, (error, stdout, stderr) => {
      ...
      exec(comandoPowerBI, (error, stdout, stderr) => {
        if (error) {
          ...
          res
            .status(500)
            .send("Error al ejecutar el proceso de PowerBI Plan de Reposición");
          return;
        }
...
app.post("/runOverridePlanReposicion", async (req, res) => {
  try {
    ...
    for (const [sku, pallets] of Object.entries(overrideValues)) {
      console.log(
        `Actualizando el documento con SKU: ${sku}, con Plan_Firme_Pallets:`,
        pallets
      );
      ...
      console.log(
        `Documentos coincidentes para el SKU: ${sku}`,
        result.matchedCount
      );
      console.log(
        `Documentos modificados para el SKU: ${sku}`,
        result.modifiedCount
      );

      if (result.matchedCount === 0) {
        console.warn(
          `Advertencia: No se encontró ningún documento para el SKU: ${sku}`
        );
      }

      if (result.modifiedCount === 0) {
        console.warn(
          `Advertencia: No se modificó ningún documento para el SKU: ${sku}`
        );
      }
    }
    ...
    const overrideValuesString = JSON.stringify(overrideValues).replace(
      /"/g,
      '\\"'
    );
  ...
  try {
    ...
    for (const [sku, overrideValue] of Object.entries(overrideValues)) {
      if (isNaN(overrideValue)) {
        console.warn(
          `Valor no numérico para SKU: ${sku}, valor: ${overrideValue}`
        );
        continue; // Saltar a la siguiente iteración si el valor no es un número
      }

      console.log(
        `Actualizando SKU: ${sku}, con Plan_Firme_Pallets: ${overrideValue}`
      );
      ...
      if (result.modifiedCount === 0) {
        console.warn(
          `Advertencia: No se modificó ningún documento para SKU: ${sku}`
        );
      }

      console.log(
        `Actualización completada en plan_reposicion_01_sem para SKU: ${sku}`
      );
    }
    ...
    res
      .status(500)
      .send("Error al procesar el override plan de reposición diario");
  }
});
...
app.post("/runOverridePoliticaInventarios", async (req, res) => {
  try {
    ...
    for (const [sku, overrideValue] of Object.entries(overrideValues)) {
      console.log(
        `Actualizando SKU: ${sku}, con Override_SS_Cantidad: ${overrideValue}`
      );
      ...
      // Verificar si la actualización fue exitosa
      console.log(
        `Documentos coincidentes para SKU: ${sku}:`,
        result.matchedCount
      );
      console.log(
        `Documentos modificados para SKU: ${sku}:`,
        result.modifiedCount
      );
      ...
      console.log(
        `Actualización completada en politica_inventarios_01 para SKU: ${sku}`
      );
    }
    ...
    res
      .status(500)
      .send("Error al procesar el override politica de inventario diario");
  }
});
...
app.post("/runOverridePoliticaInventarios_Sem", async (req, res) => {
  try {
    ...
    for (const [sku, overrideValue] of Object.entries(overrideValues)) {
      ...
      console.log(
        `Actualización completada en politica_inventarios_01_sem para SKU: ${sku}`
      );
    }
    ...
    res
      .status(500)
      .send("Error al procesar el override politica de inventario semanal");
  }
});
...
app.post("/applyGeneralOverride", async (req, res) => {
  try {
    ...
    if (!["Diario", "Semanal"].includes(tipoProceso)) {
      return res
        .status(400)
        .send('Tipo de proceso no soportado. Use "Diario" o "Semanal".');
    }
    ...
    for (const [sku, value] of Object.entries(overrideValues)) {
      ...
      if (isNaN(value)) {
        return res
          .status(400)
          .send(`El valor para el SKU ${sku} no es un número válido.`);
      }
    }
    ...
    for (const [sku, overrideValue] of Object.entries(overrideValues)) {
      try {
        console.log(
          `Procesando SKU: ${sku}, valor de ajuste: ${overrideValue}`
        );
    ...
    exec(comando, (error, stdout, stderr) => {
      ...
      if (error) {
        ...
        return res
          .status(500)
          .send(`Error al ejecutar el general override: ${stderr}`);
      }
...
app.post("/runOverridePoliticaInventarios_ROP", async (req, res) => {
  try {
    ...
    for (const [sku, overrideValue] of Object.entries(overrideValues)) {
      console.log(
        `Actualizando SKU: ${sku}, con Override_ROP: ${overrideValue}`
      );
...
app.post("/applyGeneralOverride_ROP", async (req, res) => {
  try {
    ...
    for (const [sku, overrideValue] of Object.entries(overrideValues)) {
      try {
        console.log(
          `Procesando SKU: ${sku}, valor de ajuste ROP: ${overrideValue}`
        );
    ...
    exec(comando, (error, stdout, stderr) => {
      ...
      res
        .status(200)
        .send({ resultados, mensaje: "Override ROP aplicado con éxito" });
    });
  } catch (err) {
...
app.post("/revertGeneralOverride", async (req, res) => {
  ...
  try {
    ...
    const collections = [
      {
        source:
          tipoProceso === "Diario"
            ? "ui_all_pol_inv_backup"
            : "ui_sem_all_pol_inv_backup",
        target:
          tipoProceso === "Diario" ? "ui_all_pol_inv" : "ui_sem_all_pol_inv",
      },
      {
        target:
          tipoProceso === "Diario"
        ...
        source:
          tipoProceso === "Diario"
            ? "politica_inventarios_01_backup"
            : "politica_inventarios_01_sem_backup",
      },
    ];
    ...
    for (const { source, target } of collections) {
      ...
      console.log(
        `Procesando ${documents.length} documentos desde ${source} hacia ${target}.`
      );

      // Limpiar la colección destino y restaurar documentos
      await targetCollection.deleteMany({});
      const result = await targetCollection.insertMany(
        documents.map(({ _id, ...doc }) => doc)
      );

      console.log(
        `Se copiaron ${result.insertedCount} documentos a la colección destino (${target}).`
      );
    }
...
app.post("/getPwBiUrl", async (req, res) => {
  try {
    ...
    console.log("Datos de Power BI leídos desde el archivo:", {
      PwBiUser,
      PwBiPassword,
      PwBiURL,
    });
...
// Solo corre Monte Carlo (sin override)
app.post("/runMontecarloAndOverride", async (req, res) => {
  try {
    ...
    // 2) Conectar a la base y establecer datos del usuario (para logging/consistencia)
    await conex.connectToDatabase();
    conex.setUserData(appUser, decryptedAppPass, "btc_opti_" + DBName);
    ...
    res.send({
      message:
        "Proceso de Monte Carlo completado con éxito. (No se aplicó override)",
    });
  } catch (error) {
    console.error("Error en el endpoint:", error);
    res.status(500).send({ error: "Error ejecutando Monte Carlo." });
  }
});
...
app.post("/upload", upload.single("file"), (req, res) => {
  ...
  fs.access(filePath, fs.constants.F_OK, (err) => {
    ...
    const loadCsvScriptPath = path.join(
      __dirname,
      "modulo_demanda",
      "Gestion_informacion",
      "load_data_demand.js"
    ...
    exec(command, (error, stdout, stderr) => {
      ...
      res
        .status(200)
        .send(
          `Archivo CSV para la tabla "${table}" procesado y subido correctamente a MongoDB.`
        );
    });
  });
});
...
app.post("/download-collection", (req, res) => {
  ...
  console.log(
    ...
    "selectedCollection:",
    selectedCollection
  ); // Verifica si los parámetros se están recibiendo correctamente

  const exportScriptPath = path.join(
    __dirname,
    "modulo_demanda",
    "Gestion_informacion",
    "export_data_demand.js"
  );
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    const csvFilePath = path.join(
      __dirname,
      `modulo_demanda/exports/${selectedCollection}_${appUser}.csv`
    );
...
app.post("/update-collection", upload.single("file"), (req, res) => {
  ...
  console.log(
    `Colección seleccionada para actualización: ${selectedCollection}`
  );
  ...
  fs.access(filePath, fs.constants.F_OK, (err) => {
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        return res
          .status(500)
          .send("Error al actualizar la colección seleccionada.");
      }
      if (stderr) {
        console.error(`Error en la ejecución del script: ${stderr}`);
        return res.status(500).send("Error durante la ejecución del script.");
...
app.get("/api/:selectedTable", (req, res) => {
  ...
  if (!appUser || !DBName || !selectedTable) {
    return res
      .status(400)
      .send("Faltan parámetros appUser, DBName o selectedTable.");
  }
  ...
  const outputFile = path.join(
    ...
    "modulo_demanda",
    "temp",
    `${selectedTable}_${appUser}_${Date.now()}.json`
  );
...
app.post("/api/forecast/calculate-metrics", (req, res) => {
  ...
  exec(command, { maxBuffer: 1024 * 1024 * 10 }, (error, stdout, stderr) => {
    if (error) {
      ...
      return res
        .status(500)
        .send("Error al calcular las métricas del forecast.");
    }
    if (stderr) {
      console.error("Error en el script:", stderr);
      return res
...
app.post("/runClassification", (req, res) => {
  ...
  const execPath = path.join(
    __dirname,
    "modulo_demanda",
    "Clasificador_Demanda",
    "exec_clasificador_demanda.js"
  );
  ...
  exec(command, (error, stdout, stderr) => {
    if (error) {
      ...
      return res
        .status(500)
        .send(`Error al ejecutar la clasificación: ${error.message}`);
    }
    ...
    return res
      .status(200)
      .send("Proceso de clasificación ejecutado correctamente.");
  });
});
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
app.get("/obtenerClasificacion", (req, res) => {
  ...
  exec(command, (error, stdout, stderr) => {
    if (error) {
      ...
      return res
        .status(500)
        .send(`Error al obtener datos de clasificación: ${error.message}`);
    }
    ...
    try {
      ...
      return res
        .status(500)
        .send(`Error al parsear datos: ${parseError.message}`);
    }
  });
});
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
app.get("/logs", async (req, res) => {
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] Usuario ${appUser} accedió a los logs: ${fileName}`
    );
...
app.post("/getPoliticasGuardadas", async (req, res) => {
  ...
  try {
    ...
    if (!data.length) {
      return res
        .status(404)
        .json({ error: "No se encontraron políticas guardadas" });
    }
      ...
      if (document.politica && Array.isArray(document.politica)) {
        document.politica.forEach((politica, polIndex) => {
          try {
            ...
            console.error(
              `Error procesando política ${polIndex} del documento ${docIndex}:`,
              error
            );
...
app.post("/runUsoPoliticaGuardada", async (req, res) => {
  try {
    const { appUser, appPass, DBName, idPolitica, modoUso, comentario } =
      req.body;
    ...
    console.log(
      `[runUsoPoliticaGuardada] Ejecutando modo: ${modoUso} para política ID: ${idPolitica}`
    );
    const decryptedAppPass = await getDecryptedPassUser(appPass);
    ...
    const directorioActual = __dirname;
    const rutaDirUsoPoliticaGuardada = path.join(
      directorioActual,
      "Uso_Politica_Guardada"
    );
    ...
    exec(comando, (error, stdout, stderr) => {
      if (error) {
        console.error(
          "[runUsoPoliticaGuardada] Error durante la ejecución:",
          error
        );
        return res
          .status(500)
          .send("Error al ejecutar el proceso de política guardada");
      }
...
app.post("/runCambiosIncrementales", async (req, res) => {
  try {
    ...
    console.log(
      "Ejecutando proceso incremental (P23 + P24) para usuario:",
      usuarioLog
    );
...
// Ejecuta el Python para refrescar resultados_simulaciones
async function ejecutarMontecarloPython(
  dbName,
  DBUser,
  passadminPlain, // 👈 ya desencriptado
  tipoProceso
) {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, "montecarlo", "outputAExcel.py");

    console.log(`[MC] Ejecutando Montecarlo con ${tipoProceso}...`);

    const py = spawn(
      "py",
      [scriptPath, dbName, DBUser, passadminPlain, host, puerto, tipoProceso],
      {
        cwd: path.join(__dirname, "montecarlo"),
        stdio: "inherit",
      }
    );
    ...
    py.on("close", (code) => {
      if (code === 0) {
        console.log("[MC] Montecarlo finalizado con éxito");
        resolve();
      } else {
        reject(new Error(`[MC] Montecarlo terminó con código ${code}`));
      }
    });
  });
}

async function inyectarSSdeMontecarloEnPoliticaBase({
  DBUser,
  DBPassword, // encriptado
  DBName,
  tipoProceso,
}) {
  const passadminDeCripta = await decryptData(DBPassword); // desencripta
  const uri = `mongodb://${encodeURIComponent(DBUser)}:${encodeURIComponent(
    passadminDeCripta
  )}@${host}:${puerto}/?authSource=admin`;
  const client = new MongoClient(uri);
  const dbNameFull = `btc_opti_${DBName}`;
  const esSemanal = (tipoProceso || "").toLowerCase() === "semanal";
  ...
  try {
    ...
    const db = client.db(dbNameFull);

    const simulaciones = await db
      .collection("resultados_simulaciones")
      .find({ "Normal?": "No" }) // slow-movers
      .toArray();

    if (!simulaciones.length) {
      console.log("[MC→POL] No hay slow-movers para inyectar SS.");
      return;
    }

    const politBase = db.collection(
      esSemanal ? "politica_inventarios_01_sem" : "politica_inventarios_01"
    );

    let actualizados = 0;
    for (const sim of simulaciones) {
      const mejor = (sim.mejor || "").toLowerCase();
      const col = columnas[mejor];
      if (!col) continue;
      const valorSS = sim[col];
      if (valorSS == null) continue;

      const r = await politBase.updateOne(
        { SKU: sim.SKU },
        { $set: { SS_Cantidad: valorSS, FromMontecarlo: true } } // 👈 flag agregado
      );
      if (r.matchedCount) actualizados++;
    }

    console.log(
      `[MC→POL] SS inyectado en ${actualizados} SKU(s) de ${politBase.collectionName}.`
    );
  } finally {
    await client.close();
  }
...
async function getDBCredsFor(appUser, DBName) {
  // Conecta con el mismo cliente que ya usas
  const client = await conex.connectToDatabase();
  const dbUsuarios = client.db("OptiBTC").collection("usuarios");

  // Busca documento del propietario principal o del que te incluya en UserUI
  const user = await dbUsuarios.findOne({
    $or: [{ AppUser: appUser }, { "UserUI.AppUser": appUser }],
  });

  if (!user)
    throw new Error(`Usuario ${appUser} no encontrado en OptiBTC.usuarios`);

  // ¿eres main user?
  if (user.AppUser === appUser) {
    // en el /login, para main user: dbName = `btc_opti_${p_AppUser}`
    const userDB =
      user.UserDB?.find((db) => db.DBName === `btc_opti_${DBName}`) ||
      user.UserDB?.[0];

    if (!userDB) throw new Error(`No hay mapeo UserDB para ${DBName}`);

    return {
      DBUser: userDB.DBUser,
      DBPassword: userDB.DBPassword,
    };
  }

  // si eres sub-user: tu /login toma el mapeo desde user.UserDB (por DBName exacto o el primero)
  const userDB =
    user.UserDB?.find((db) => db.DBName === `btc_opti_${DBName}`) ||
    user.UserDB?.[0];
...
async function prepararSSAntesDePolitica({
  appUser,
  appPass,
  DBName,
  tipoProceso,
}) {
  const { DBUser, DBPassword } = await getDBCredsFor(appUser, DBName);
  const dbUserPlain = DBUser;
  const dbPassPlain = await decryptData(DBPassword); // plano para conectar

  // 1) Ejecutar Montecarlo con pass plano
  await ejecutarMontecarloPython(
    `btc_opti_${DBName}`,
    dbUserPlain,
    dbPassPlain, // 👈 plano → Python
    tipoProceso
  );

  // 2) Inyectar SS en políticas con pass encriptado
  await inyectarSSdeMontecarloEnPoliticaBase({
    DBUser: dbUserPlain,
    DBPassword, // 👈 encriptado → Node/Java
    DBName,
    tipoProceso,
  });
}
