...
// ----------------------
// Registro dinámico de tenants activos (en memoria)
// ----------------------
const ACTIVE_TENANTS = new Set();
/**
 * Clave canónica de tenant, evita duplicados.
 */
function tenantKey(appUser, dbName) {
  return `${appUser}::${dbName}`;
}

/**
 * Agrega el tenant visto en runtime.
 */
function registerTenant(appUser, dbName) {
  ACTIVE_TENANTS.add(tenantKey(appUser, dbName));
}

/**
 * Itera sobre los tenants registrados y ejecuta cleanup en cada uno.
 */
...
console.log(`Modulo de demanda:Planner >> Sesion cerrada debido a inactividad. Base de datos: ${dbName}:`, closed);
      ...
      // no hacemos throw: seguimos con los demás tenants
    }
  }
}
...
setInterval(() => {
  ...
  sweepAllTenants(GRACE_SECONDS);
}, CLEANUP_INTERVAL_MS);

// === [Planner] BEGIN middleware ===
app.use("/planner", async (req, res, next) => {
  try {
    const appUser =
      req.header("x-app-user") || req.body.appUser || req.query.appUser;

    let dbName =
      req.header("x-db-name") ||
      req.body.DBName ||
      req.body.dbName ||
      req.query.DBName ||
      req.query.dbName;
    ...
    if (!appUser || !dbName) {
      return res.status(400).json({ error: "Faltan appUser o DBName" });
    }

    // Normaliza si viene sin prefijo
    if (!/^btc_opti_/i.test(dbName)) {
      dbName = `btc_opti_${dbName}`;
    }

    // Obtén DB para este request
    req.db = await getDb(appUser, dbName);
    req.appUser = appUser;

    // Asegura índices (idempotente)
    await ensurePlannerIndexes(req.db);

    // Registra el tenant para el sweep periódico
    registerTenant(appUser, dbName);

    return next();
  } catch (err) {
    console.error("[Planner] middleware error:", err);
    return res.status(500).json({ error: err.message });
  }
});

// Monta endpoints del Planner
...
app.get("/obtenerClasificacion", (req, res) => {
  ...
  exec(command, { maxBuffer: 1024 * 2048 * 10 }, (error, stdout, stderr) => {
    if (error) {
      ...
      return res
        .status(500)
        .send("Error al obtener los datos de la clasificacion");
    }
    if (stderr) {
